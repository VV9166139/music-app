    ...
    <ToastContext.Provider value={{ showToast }}>
      ...
      <div className="fixed top-5 right-5 z-[9999] flex flex-col gap-2.5 max-w-sm w-full pointer-events-none px-4">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`pointer-events-auto flex items-center justify-between gap-3 px-4 py-3 rounded-xl shadow-2xl backdrop-blur-xl border transition-all duration-300 animate-in slide-in-from-top-3 ${
              toast.type === 'success'
                ? 'bg-[#0E1B17]/90 border-emerald-500/30 text-emerald-300'
                : toast.type === 'error'
                ? 'bg-[#221013]/90 border-rose-500/30 text-rose-300'
                : 'bg-[#12182B]/90 border-indigo-500/30 text-indigo-300'
            }`}
          >
